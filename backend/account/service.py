...
class AccountService:
    ...
    def get_order_history(cls, user: user_models.User):
        orders = sales_models.Order.objects.filter(user=user).order_by(
            "-created_at"
        )
        return orders
    ...
    def get_course_modules(cls, course: course_models.Course):
        modules = course_models.Module.objects.filter(course=course).order_by(
            "order"
        )
        return modules
    ...
    def get_module_contents(cls, module: course_models.Module):
        contents = course_models.Content.objects.filter(
            module=module
        ).order_by("order")
        return contents
    ...
    def get_content(cls, kwargs: dict, course: course_models.Course):
        ...
        try:
            return course_models.Content.objects.get(
                id=content, module__course=course
            )
    ...
    def get_announcement_detail(
        cls, course: course_models.Course, kwargs: dict
    ):
        try:
    ...
    def get_announcement_comments(
        cls, announcement: course_models.CourseAnnouncement
    ):
    ...
    def get_course_questions(cls, course: course_models.Course):
        return course_models.CourseQuestion.objects.filter(
            course=course
        ).order_by("-created_at")

    @classmethod
    def get_course_questions_comment(
        cls, question: course_models.CourseQuestion
    ):
    ...
    def get_content_notes(
        cls, user: user_models.User, content: course_models.Content
    ):
    ...
    def get_course_chat_room(cls, course: course_models.Course):
        chat_room = course_models.CourseChatRoom.objects.get_or_create(
            course=course
        )
        return chat_room
    ...
    def get_course_chat_messages(cls, course: course_models.Course):
        ...
        messages = course_models.CourseMessage.objects.filter(
            room=chat_room
        ).order_by("-created_at")
        return messages
